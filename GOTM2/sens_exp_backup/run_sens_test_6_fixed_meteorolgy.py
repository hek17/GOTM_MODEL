...
def rewrite_gotm_run(orig_nml, gotm_run, yy, mm, dd, tt, lat, lon, outname):
   ...
   for i, line in enumerate(info):
      if line.startswith('&output'):
         ...
         data[i+2]= "   latitude = "+ lat +",\n"
         data[i+3]= "   longitude =" + lon +",\n"
      else:
         continue
   # end if
...
for i in range(0,len(chl_arr),1):
   ...
   for t, time in enumerate(intensities):
      ...
      rewrite_gotm_run(orig_nml, gotm_run, yy, mm, dd, tt, lat, lon, gotm_outfile)  
      ...
      extf   = '/'+ ext_name
      rm_arr = [extf, "/gotmrun.nml", "/obs.nml" ]
